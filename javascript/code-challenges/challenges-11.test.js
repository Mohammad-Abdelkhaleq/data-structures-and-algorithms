...
const count = (target, input) => {
  ...
  let newArr = [];
  input.forEach((value) => {
    newArr.push(value.filter((num) => num === target));
  }
  );
...
const totalSum = (input) => {
  ...
  let newArr = [];
  input.forEach((value) => {
    newArr.push(value.reduce((acc, val) => acc + val));
  }
  );
...
function divisibleByFiveTwoToThePower(arr) {
  return arr.map(subArray => {
    return subArray.filter(element => typeof element === 'number' && element % 5 === 0)
...
let findShortest = (data) => {
  ...
  let newArr3 = newArr2.sort((a, b) => a - b);
  let newArr4 = newArr3.filter((value) => value === newArr3[0]);
  let newArr5 = [];
  data.forEach((value) => {
    if (value.height === newArr4[0].toString()) {
